"""M1-05 公共夹具：独立临时 SQLite 空库、运行数据根、导入网关与查询网关。

复用 M1-03/04 的迁移（Alembic head）、外键强制与目录约定（规格 11.3 /
ADR-0033）：formal_root 指向临时运行数据根，其下 batches/<batch_id>/
<data_version>/ 为正式批次目录；tmp_root 为独立临时导入根。query_gateway
注入 CaseQueryGateway（同一 formal_root，读取已保存的 source_filename.txt）。
"""
...
def gateway(engine: Engine, formal_root: Path, tmp_root: Path) -> BatchImportGateway:
    ...
    return BatchImportGateway(
        engine=engine, formal_root=formal_root, tmp_root=tmp_root
    )


@pytest.fixture
def query_gateway(engine: Engine, formal_root: Path) -> CaseQueryGateway:
    """注入独立库与同一正式运行数据根的 CaseQueryGateway。"""
    return CaseQueryGateway(engine=engine, formal_root=formal_root)
