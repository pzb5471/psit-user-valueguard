"""M1-04 公共夹具：独立临时 SQLite 空库、运行数据根与 BatchImportGateway。

每次测试独立临时库（Alembic head + 外键强制）；formal_root 指向临时运行数据根，
其下 batches/<batch_id>/<data_version>/ 为正式批次目录（规格 11.3 / ADR-0033）；
tmp_root 指向独立临时导入根（规格 11.3 imports/tmp/）。ZIP 构造助手见 zip_fixtures.py。
"""
