...
4. 两名验收者独立检查 5 个密封案例，在 PoC 报告中记录人工结论。自动结构门不能代替双人质量判断。
5. 只有自动结构门与双人复核均通过后，才能在受审查提交中把 `config/default.toml` 的 `analysis.production_enabled` 改为 `true`。
