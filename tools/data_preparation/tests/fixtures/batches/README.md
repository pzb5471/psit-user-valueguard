...
本目录不存放 ZIP 产物：MVP/ACCEPTANCE 两个标准 ZIP 属于运行数据，由组包程序
...
| 批次 | 案例数 | ZIP 文件名 | 说明 |
|---|---|---|---|
| MVP | 10 | `mvp_batch_v1.zip` | 规格 5.5 演示批次（含图片案例） |
| ACCEPTANCE | 5 | `acceptance_batch_v1.zip` | 验收批次（000003/000006/000009/000051/000074） |

推荐重建命令（仓库根目录，会自动发现仓库同级源数据并执行 M1-09）：

    powershell -NoProfile -ExecutionPolicy Bypass -File .\scripts\prepare-mvp-data.ps1

源数据位于其他目录时：

    powershell -NoProfile -ExecutionPolicy Bypass -File .\scripts\prepare-mvp-data.ps1 -SourceRoot <逻辑源目录>
